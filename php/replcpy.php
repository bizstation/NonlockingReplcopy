...
const DYNAMIC_REPLCOPY_VERSION = "1.0.1";
...
function getSlaveUri($param, $db)
{
    return getUri($param, "slave", $db);
}

function getMasterUri($param, $db)
{
    return getUri($param, "master", $db);
}
...
{
    if ($param["gtid"]["type"] == REP_TYPE_REGACY) {
        return getChangeMasterSqlPos($param, $binlog);
    }
    if ($param["gtid"]["type"] == REP_TYPE_GTID_MA) {
        return getChangeMasterSqlGtidMa($param);
    }
    if ($param["gtid"]["type"] == REP_TYPE_GTID_MY) {
        return getChangeMasterSqlGtidMy($param);
    }
}
...
function createOpen($db, $uri)
{
    ...
    $db->open($uri);
    if ($db->stat() == Transactd::ERROR_NO_DATABASE && ($dbname != "mysql")) {
        $db->create($uri);
...
function setSqlMode($db)
{
    execSql($db, "set session sql_mode=''");
}

function setLogBinOff($db)
{
    execSql($db, "set session sql_log_bin=OFF");
}
...
function openMasterDb($db, $param)
{
    ...
    $n = count($uris);
    if ($n == 0) {
        throw new Exception("No replication databases");
    }
    ...
    foreach ($uris as $uri) {
        ...
        $dbtmp->open($uri, Transactd::TYPE_SCHEMA_BDF, Transactd::TD_OPEN_READONLY);
...
function stopSlave($db)
{
    execSql($db, "stop slave");
}

function resetSlave($db)
{
    execSql($db, "reset slave all");
}

function startSlave($db)
{
    execSql($db, "start slave");
}
...
function changeMaster($db, $param, $binlog)
{
    ...
    } elseif ($gtidType == REP_TYPE_GTID_MY) {
...
function openMasterTables($dbs, $param)
{
    ...
    for ($j = 0; $j < $dbcount; ++$j) {
        ...
        foreach ($tableNames as $name) {
            $tbs[] = $dbs[$j]->openTable($name, Transactd::TD_OPEN_READONLY);
...
{
    $binlog = new BinlogPos();
    $binlog = $db->beginSnapshot(Transactd::CONSISTENT_READ_WITH_BINLOG_POS);
...
function slaveSync($db, $mgr, $binlog, $mysqlGtidMode)
{
    while (1) {
        ...
        if ((isSlaveIoRunning($recs) === false) && getSlaveIoErrno($recs)) {
            if (resolvIOError($db, $recs)) {
                return true;
            }
        } elseif (isSlaveSqlRunning($recs) == false) {
            if (resolvSqlError($db, $recs, $binlog, $mysqlGtidMode)) {
                return true;
            }
        }
    }
...
function isMysqlGtidModeOn($param, $db)
{
    $vers = new BtrVersions;
...
function waitForSlaveSync($db, $param, $binlog, $mgr)
{
    ...
    $mysqlGtidMode = isMysqlGtidModeOn($param, $db);
...
{
    return ($name == Transactd::TRANSACTD_SCHEMANAME) ||
...
function main($argc, $argv)
{
    try {
        ...
        $db_master = new Database();
        $db_slave = new Database();
